...
const PLAN_CONFIG = {
  free:  { price: 0,    tokenLimit: 10000,   billingCycle: 'lifetime', features: ['chat'] },
  basic: { price: 999,  tokenLimit: 100000,  billingCycle: 'monthly', features: ['chat', 'assignments', 'documents'] },
  pro:   { price: 1999, tokenLimit: 500000,  billingCycle: 'monthly', features: ['chat', 'assignments', 'documents', 'quiz', 'flashcards', 'mcqs'] },
};
...
exports.getPlans = (req, res) => {
  res.json({ 
    success: true, 
    plans: PLAN_CONFIG,
    note: "Basic and Pro are monthly subscriptions - tokens reset each month. Free plan is lifetime."
  });
};
  ...
  try {
    ...
    res.status(200).json({
      ...
      usage: {
        ...
        tokenLimit: usage?.tokenLimit || 10000,
        remainingTokens: (usage?.tokenLimit || 10000) - (usage?.totalTokens || 0),
      },
    });
  } catch (error) {
    next(error);
...
exports.paymentWebhook = async (req, res, next) => {
  try {
    ...
    if (eventName === 'subscription_created' || eventName === 'subscription_updated') {
      ...
      await Usage.findOneAndUpdate({ userId }, { tokenLimit: 10000 });
